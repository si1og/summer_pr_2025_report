...
\scalebox{0.8} {
  ...
  % \draw[->] (assign) -- ++(0, -1) |- (loop.west);
  % \draw[->] (cond) -- node[left] {нет} ++(0, 1) -- (0, 0) |- (loop.west);
  \draw[->] (ret) -- (stop);
...
\scalebox{0.8}{
  ...
  \node (input_msg) [right of=start, yshift=-1.5cm] {Выход: };
  \node (return_msg) [below of=input_msg, yshift=1.4cm] {Вывод: };

  \node (check_is_below_0) [draw, diamond, below of=start, aspect=2, yshift=-2cm, yshift=-2cm] {size <= 0};
  ...
  \draw[->] (start) -- ++(0, -2) -- ++(1.1,0) -- ++(0,1) -- ++(0.2,0);
  \draw[->] (start) -- ++(0, -2) -- ++(1.1,0) -- ++(0,-1) -- ++(0.2,0);
